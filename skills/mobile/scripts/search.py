#!/usr/bin/env python3
"""
MobileSkill — CLI Search Entry Point

Usage:
    # Domain search
    python3 search.py "MVVM pattern" --domain pattern
    python3 search.py "dio http client" --domain package
    python3 search.py "certificate pinning" --domain security

    # Stack search
    python3 search.py "state management bloc" --stack flutter
    python3 search.py "navigation hooks" --stack react-native

    # Auto-detect domain
    python3 search.py "flutter bloc state management"

    # Architecture system
    python3 search.py "flutter e-commerce app" --arch-system -p "ShopApp"
    python3 search.py "react native social media" --arch-system --persist -p "SocialApp"
...
def main():
    parser = argparse.ArgumentParser(
        description="MobileSkill - Search mobile development best practices, patterns, and architecture",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 search.py "MVVM pattern" --domain pattern
  python3 search.py "secure storage keychain" --domain security
  python3 search.py "state management" --stack flutter
  python3 search.py "navigation hooks" --stack react-native
  python3 search.py "flutter e-commerce app" --arch-system -p "ShopApp"
        """
    )
    ...
    parser.add_argument("--project", "-p", default="MyApp",

"""
Architecture System Generator for RustSkill
Generates complete Rust project architecture recommendations by combining
...
REASONING_FILE = DATA_DIR / "rust-reasoning.csv"


def load_reasoning(query):
    """Load and match reasoning rules from rust-reasoning.csv."""
...
def _format_box(selected, project_name):
    ...
    hr()
    row(f"TARGET: {project_name} - RECOMMENDED RUST ARCHITECTURE")
    hr()
    ...
    # Also include crate search results
    pkg_results = selected.get("crate_results", [])
    if pkg_results:
    ...
    if arch_results:
        ...
        # Default Rust project structure
        for d in [
            "src/", "   main.rs", "   lib.rs",
    ...
    row("   Stage 1: rust:1.85-slim -> build with cargo-chef")
    row("   Stage 2: debian:bookworm-slim or distroless -> copy binary only")
...
def _format_markdown(selected, project_name):
    """Format as markdown."""
    lines = []
    lines.append(f"# {project_name} - Rust Architecture Recommendation")
    lines.append("")
    ...
    if arch_results:
        ...
        lines.append("src/lib.rs")
        lines.append("src/config/")
        lines.append("src/handlers/")
        lines.append("src/services/")
        lines.append("src/repositories/")
        lines.append("src/models/")
        lines.append("src/errors/")
        lines.append("migrations/")
        lines.append("Cargo.toml")
...
def generate_arch_system(query, project_name="MyProject", format_type="box", persist=False, page=1, output_dir=None):
    ...
    2. Load reasoning rules from rust-reasoning.csv
    ...
    search_results = {}
    for domain, max_r in [("crate", 3), ("pattern", 3), ("error", 2), ("arch", 2), ("concurrency", 2), ("perf", 2)]:

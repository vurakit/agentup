"""
Architecture System Generator for PHPSkill
Generates complete PHP project architecture recommendations by combining
...
REASONING_FILE = DATA_DIR / "php-reasoning.csv"


def load_reasoning(query):
    """Load and match reasoning rules from php-reasoning.csv."""
...
def apply_reasoning(reasoning, search_results):
    ...
    selected["architecture"] = reasoning.get("Recommended_Architecture", "MVC (Laravel)")
...
def _format_box(selected, project_name):
    ...
    hr()
    row(f"TARGET: {project_name} - RECOMMENDED PHP ARCHITECTURE")
    hr()

    # Architecture
    section("ARCHITECTURE")
    arch = selected.get("architecture", "MVC (Laravel)")
    row(f"   {arch}")

    # Packages
    section("CORE PACKAGES (Composer)")
    ...
    if arch_results:
        ...
        for d in [
            "app/", "   Http/Controllers/", "   Http/Middleware/",
            "   Models/", "   Services/", "   Repositories/",
            "   Exceptions/", "config/", "database/",
            "   migrations/", "   seeders/", "routes/",
            "resources/views/", "tests/", "Dockerfile",
    ...
    for cmd in [
        "serve:     php artisan serve / symfony server:start",
        "test:      php artisan test / ./vendor/bin/phpunit",
        "lint:      ./vendor/bin/phpstan analyse",
        "format:    ./vendor/bin/pint / ./vendor/bin/php-cs-fixer fix",
    ]:
        row(f"   {cmd}")

    # Dockerfile
    section("DOCKERFILE (multi-stage)")
    row("   Stage 1: composer:2 -> install dependencies")
    row("   Stage 2: php:8.3-fpm-alpine -> copy app + vendor")
    row("   Nginx: nginx:alpine -> reverse proxy to PHP-FPM")
...
def _format_markdown(selected, project_name):
    """Format as markdown."""
    lines = []
    lines.append(f"# {project_name} - PHP Architecture Recommendation")
    lines.append("")
    ...
    # Packages
    lines.append("## Core Packages (Composer)")
    lines.append("")
    packages = selected.get("packages", "")
    if packages:
        for pkg in packages.split(","):
    ...
    if arch_results:
        ...
        lines.append("app/Http/Controllers/")
        lines.append("app/Http/Middleware/")
        lines.append("app/Models/")
        lines.append("app/Services/")
        lines.append("app/Repositories/")
        lines.append("config/")
        lines.append("database/migrations/")
        lines.append("routes/api.php")
        lines.append("resources/views/")
        lines.append("tests/")
...
def generate_arch_system(query, project_name="MyProject", format_type="box", persist=False, page=1, output_dir=None):
    ...
    2. Load reasoning rules from php-reasoning.csv

#!/usr/bin/env python3
"""
PHPSkill — CLI Search Entry Point

Usage:
    # Domain search
    python3 search.py "repository pattern" --domain pattern
    python3 search.py "composer package auth" --domain package
    python3 search.py "sql injection prevention" --domain security

    # Stack search
    python3 search.py "middleware authentication" --stack laravel
    python3 search.py "doctrine entity mapping" --stack symfony

    # Auto-detect domain
    python3 search.py "phpunit mocking test"

    # Architecture system
    python3 search.py "laravel rest api with mysql" --arch-system -p "MyAPI"
    python3 search.py "symfony microservice" --arch-system --persist -p "MySfService"
...
def main():
    parser = argparse.ArgumentParser(
        description="PHPSkill - Search PHP best practices, patterns, and architecture",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 search.py "repository pattern" --domain pattern
  python3 search.py "authentication jwt" --domain package
  python3 search.py "sql injection" --domain security
  python3 search.py "middleware auth" --stack laravel
  python3 search.py "rest api mysql" --arch-system -p "MyAPI"
        """
    )
